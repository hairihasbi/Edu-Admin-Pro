...
import { 
  Shield, Activity, IdCard, Settings, Search, 
  Trash2, RotateCcw, CheckCircle, AlertTriangle,
  Server, Smartphone, Zap, Clock, Save, Info
} from './Icons';
...
const RfidSecurityManager: React.FC<RfidSecurityManagerProps> = ({ user }) => {
  ...
  const [activeDevices, setActiveDevices] = useState<{id: string, lastSeen: string, method: string}[]>([]);
  ...
  const handleBlockCard = async (student: Student) => {
    if (!window.confirm(`Blokir kartu untuk ${student.name}? Kartu lama tidak akan bisa digunakan lagi.`)) return;
    ...
    const updatedSettings = {
      ...settings!,
      rfidBlockedTags: blockedTag ? [...currentBlocked, blockedTag] : currentBlocked
    };
    ...
    if (success) {
      ...
      setStudents(prev => prev.map(s => s.id === student.id ? { ...s, rfidTag: '' } : s));
    }
  };
  ...
  const handlePairCard = async () => {
    ...
    setSaving(true);
    try {
      const success = await updateStudentRfid(pairingStudentId, newTagId.trim());
      if (success) {
        setStudents(prev => prev.map(s => s.id === pairingStudentId ? { ...s, rfidTag: newTagId.trim() } : s));
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
          <div>
            <h2 className="text-xl font-bold text-gray-800">Manajemen & Keamanan RFID</h2>
            <p className="text-sm text-gray-500">Pusat konfigurasi teknis dan operasional kartu RFID sekolah.</p>
          </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
               <div className="p-4 border-b border-gray-50 bg-gray-50/50 flex justify-between items-center">
                  ...
                  <span className="px-2 py-1 bg-green-100 text-green-700 text-[10px] font-bold rounded-full">LIVE</span>
               </div>
               <div className="p-0">
                  <table className="w-full text-sm text-left">
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {activeDevices.length === 0 ? (
                        <tr>
                          <td colSpan={4} className="p-12 text-center text-gray-400 italic">Belum ada perangkat yang tercatat melakukan scanning hari ini.</td>
                        </tr>
                      ) : (
                            ...
                            <td className="p-4">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  ...
                                  <p className="text-[10px] text-gray-500">Pos Scanner Aktif</p>
                                </div>
                              </div>
                            </td>
                            <td className="p-4 text-gray-600">
                                {new Date(device.lastSeen).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}
                                <span className="block text-[10px] text-gray-400">{new Date(device.lastSeen).toLocaleDateString()}</span>
                            </td>
                            <td className="p-4">
                                <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${device.method === 'SERIAL' ? 'bg-indigo-100 text-indigo-700' : 'bg-blue-100 text-blue-700'}`}>
                                    {device.method}
                                </span>
                            </td>
                            <td className="p-4">
          ...
          <div className="space-y-6">
             <div className="bg-gradient-to-br from-blue-600 to-indigo-700 text-white p-6 rounded-2xl shadow-lg border border-blue-500/20 relative overflow-hidden">
                ...
                <p className="text-sm text-blue-100 mb-6 font-medium">
                  Pengaturan Keamanan dan Jam Absensi yang Anda ubah di sini akan otomatis sinkron ke seluruh terminal di sekolah.
                </p>
                <div className="space-y-4">
                  <div className="flex items-center gap-3 bg-white/10 p-3 rounded-xl border border-white/20">
                    ...
                    <div>
                      <p className="text-[10px] uppercase font-bold text-blue-200">Jam Masuk</p>
                      <p className="font-bold">{settings?.rfidCheckInLate || '-'} WIB</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 bg-white/10 p-3 rounded-xl border border-white/20">
                    <Shield size={24} className="text-blue-200" />
                    <div>
                      <p className="text-[10px] uppercase font-bold text-blue-200">Antrian Tap</p>
                      <p className="font-bold">{settings?.rfidCooldownSeconds || '0'} Detik</p>
                    </div>
                  </div>
                </div>
             </div>
      ...
      {activeTab === 'SECURITY' && (
        <form onSubmit={handleSaveSettings} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="p-6 border-b border-gray-100 flex justify-between items-center">
            <div>
              <h3 className="font-bold text-gray-800">Security Rules (Aturan Keamanan)</h3>
              <p className="text-xs text-gray-500">Aturan pencegahan kecurangan dan kesalahan tapping kartu.</p>
            </div>
            <button 
              disabled={saving}
              type="submit" 
          ...
          <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-4">
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-gray-500 italic">Mencegah kartu di-tap berkali-kali secara tidak sengaja.</p>
                </div>
              </div>
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1">Durasi Cooldown (Detik)</label>
                <input 
                  type="number" 
                  min="0"
                  max="60"
                  ...
                  onChange={e => setSettings(prev => prev ? ({ ...prev, rfidCooldownSeconds: parseInt(e.target.value) }) : null)}
                  placeholder="Contoh: 10"
                />
                <p className="text-[10px] text-gray-400 mt-1">*Terminal akan menolak kartu yang sama sebelum durasi ini berakhir.</p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  <h4 className="font-bold text-gray-900">Anti-Duplicate Prevention</h4>
                  <p className="text-[10px] text-gray-500 italic">Mencegah satu kartu absen di dua pos dalam waktu cepat.</p>
                </div>
              </div>
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1">Ambang Batas Duplikasi (Menit)</label>
                <input 
                  type="number" 
                  min="0"
                  max="60"
                  ...
                  onChange={e => setSettings(prev => prev ? ({ ...prev, rfidAntiDuplicateMinutes: parseInt(e.target.value) }) : null)}
                  placeholder="Contoh: 5"
                />
                <p className="text-[10px] text-gray-400 mt-1">*Mencegah tapping jika baru saja tap di alat lain dalam rentang menit ini.</p>
              </div>
            </div>
          </div>
          <div className="p-6 bg-rose-50 border-t border-rose-100">
            ...
            <div className="flex flex-wrap gap-2">
                {settings?.rfidBlockedTags && settings.rfidBlockedTags.length > 0 ? (
                    settings.rfidBlockedTags.map((tag, i) => (
                        <div key={i} className="px-3 py-1 bg-white border border-rose-200 text-rose-700 rounded-full text-[10px] font-mono font-bold flex items-center gap-2">
                            {tag}
                            <button 
                                onClick={() => setSettings(prev => prev ? ({ ...prev, rfidBlockedTags: prev.rfidBlockedTags?.filter(t => t !== tag) }) : null)}
                    ...
                    <p className="text-xs text-rose-400 italic">Belum ada kartu yang diblokir.</p>
                )}
            </div>
          </div>
        </form>
      ...
      {activeTab === 'CARD' && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="p-6 border-b border-gray-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              <h3 className="font-bold text-gray-800">Layanan & Penggantian Kartu</h3>
              <p className="text-xs text-gray-500">Tangani laporan kartu hilang dan pendaftaran ulang kartu siswa.</p>
            </div>
          ...
          <div className="bg-blue-50/50 p-4 border-b border-blue-50 flex gap-3 text-blue-800 text-[10px]">
             ...
             <p>
               <strong>Panduan Cepat:</strong> Gunakan tombol <RotateCcw size={12} className="inline"/> <strong>Pairing Ulang</strong> jika siswa membawa kartu baru. ID kartu lama akan otomatis terhapus dari data siswa tersebut. Gunakan tombol <Trash2 size={12} className="inline"/> <strong>Blokir</strong> jika kartu hilang agar ID tersebut masuk ke Daftar Hitam keamanan.
             </p>
          </div>
            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-gray-100">
                {filteredStudents.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="p-12 text-center text-gray-400 italic">Data siswa tidak ditemukan.</td>
                  </tr>
                ) : (
                    ...
                    <tr key={student.id} className="hover:bg-gray-50 transition">
                      ...
                      <td className="p-4">
                         <span className="px-2 py-0.5 bg-gray-100 text-gray-600 rounded text-[10px] font-bold uppercase">
                            {classes.find(c => c.id === student.classId)?.name || 'TPS'}
                         </span>
                      </td>
                      <td className="p-4">
                         {student.rfidTag ? (
                            ...
                            <span className="text-gray-400 italic text-xs">Belum Ada Kartu</span>
                         )}
                      </td>
                      <td className="p-4">
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
           <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-2xl space-y-6">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-bold text-gray-800">Pairing Kartu Baru</h3>
                <button onClick={() => setPairingStudentId(null)} className="text-gray-400 hover:text-gray-600">
              ...
              <div className="p-4 bg-blue-50 rounded-xl border border-blue-100 flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-blue-200 flex items-center justify-center text-blue-700 font-bold">
                   {students.find(s => s.id === pairingStudentId)?.name.charAt(0)}
                </div>
                <div>
                  <p className="text-xs font-bold text-blue-800">{students.find(s => s.id === pairingStudentId)?.name}</p>
                  <p className="text-[10px] text-blue-600">NIS: {students.find(s => s.id === pairingStudentId)?.nis}</p>
                </div>
              </div>

              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase mb-2 tracking-widest">ID Kartu (RFID Tag)</label>
                <input 
                  type="text" 
                  autoFocus
                ...
                />
                <p className="text-[10px] text-gray-400 mt-2 italic text-center italic">Tip: Tempelkan kartu pada alat saat kursor berada di kotak ini.</p>
              </div>
